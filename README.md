...
- **NB URL 分析**：輸入特定格式的網址，自動替換網域並解析參數，結果以表格呈現（包含受理號碼與目前狀態 nodeName），右側歷史紀錄可一鍵加入案件紀錄或刪除。
- **案件紀錄**：專屬重要案件留存頁面，呈現受理號碼 (applnum)、目前狀態 (nodeName)、建立時間、雙連結跳轉（原本連結 / 本機端連結）以及隨時可編輯儲存的備註欄位。
...
- **資料備份與搬移**：提供全站歷史紀錄與案件紀錄的 JSON 一鍵匯出（下載/複製）與匯入（預覽/合併/覆蓋）功能，方便更換電腦時無縫搬移資料。
- **深色/淺色模式切換**：全新設計之現代質感淺色與深色模式，支援 Google Fonts (Plus Jakarta Sans, Outfit, JetBrains Mono) 與純淨無雜訊介面。
...
- **路由**: React Router DOM
- **圖示**: React Icons
- **字體**: Google Fonts (Plus Jakarta Sans, Outfit, JetBrains Mono)
...
5.  **部署至 GitHub Pages**
    如果您需要將最新程式碼編譯並部署發布到 GitHub Pages 網站：
    ```bash
    npm run deploy
    ```
    此指令會自動執行打包並將檔案推送到 `gh-pages` 分支，等待約 1~2 分鐘後線上網站即會更新。
...
|   |-- CaseRecord.tsx         // 案件紀錄工具 (雙連結跳轉與即時備註修改)
